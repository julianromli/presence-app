...
export async function fetchWorkspaceBillingSummary() {
  ...
  if (!response.ok) {
    throw await parseApiErrorResponse(response, 'Gagal memuat billing workspace.');
  }
...
export async function fetchWorkspaceBillingInvoices() {
  const response = await workspaceFetch('/api/workspaces/current/billing/invoices', {
    cache: 'no-store',
  });

  if (!response.ok) {
    throw await parseApiErrorResponse(response, 'Gagal memuat riwayat pembayaran workspace.');
  }
...
export async function createWorkspaceCheckout(billingPhone: string) {
  const response = await workspaceFetch('/api/workspaces/current/billing/checkout', {
    method: 'POST',
  ...
  if (!response.ok) {
    throw await parseApiErrorResponse(response, 'Gagal membuat checkout workspace.');
  }
...
export async function refreshWorkspacePendingInvoice() {
  const response = await workspaceFetch('/api/workspaces/current/billing/refresh', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({}),
  });
  ...
  if (!response.ok) {
    throw await parseApiErrorResponse(response, 'Gagal menyegarkan status pembayaran workspace.');
  }
...
export async function fetchWorkspaceRestrictions() {
  const response = await workspaceFetch('/api/workspaces/current/restrictions', {
    cache: 'no-store',
  });

  if (!response.ok) {
    throw await parseApiErrorResponse(response, 'Gagal memuat status pembatasan workspace.');
  }

  return (await response.json()) as WorkspaceRestrictedExpiredStatePayload;
}

export async function normalizeWorkspaceBillingError(error: unknown, fallbackMessage: string) {
