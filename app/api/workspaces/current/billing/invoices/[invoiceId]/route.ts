...
export async function GET(
  ...
  if (!normalizedInvoiceId) {
    return Response.json(
      { code: "VALIDATION_ERROR", message: "Field invoiceId wajib diisi." },
      { status: 400 },
    );
  }

  try {
    const payload = await convex.query<WorkspaceBillingInvoiceDetailPayload>(
      "workspaceBilling:getWorkspaceBillingInvoiceDetail",
      { invoiceId: normalizedInvoiceId, workspaceId },
    );
    return Response.json(payload);
  } catch (error) {
    return convexErrorResponse(error, "Gagal memuat detail invoice workspace.");
  }
}
